...
function test_utils()
    @testset verbose = VERBOSE showtiming = SHOWTIMING "ctindice (single subscript char)" begin
    ...
    @testset verbose = VERBOSE showtiming = SHOWTIMING "ctindices (multi-digit subscript string)" begin
    ...
    @testset verbose = VERBOSE showtiming = SHOWTIMING "ctupperscript (single superscript char)" begin
    ...
    @testset verbose = VERBOSE showtiming = SHOWTIMING "ctupperscripts (multi-digit superscript string)" begin
        # Exception for negative input
    ...
    @testset verbose = VERBOSE showtiming = SHOWTIMING "ctindices / ctindice consistency" begin
        for d in 0:9
            @test CTBase.ctindices(d) == string(CTBase.ctindice(d))
        end
    end

    @testset verbose = VERBOSE showtiming = SHOWTIMING "ctupperscripts / ctupperscript consistency" begin
        for d in 0:9
