function test_function()
    ...
    @testset "BoundaryConstraint" begin
        ...
        @test_throws IncorrectArgument BoundaryConstraint(
    ...
    @testset "Mayer" begin
        ...
        @test_throws IncorrectArgument Mayer(
            (x0, xf) -> [xf[2] - x0[1], 2xf[1] + x0[2]^2], Int64)
    ...
    @testset "Hamiltonian" begin
        ...
        H = Hamiltonian(
            (x, p, v) -> x[1]^2 + 2p[2] + v[3]; autonomous = true, variable = true)
        @test H([1, 0], [0, 1], [1, 2, 3]) == 6
        @test H(t, [1, 0], [0, 1], [1, 2, 3]) == 6
        H = Hamiltonian(
            (t, x, p) -> t + x[1]^2 + 2p[2]; autonomous = false, variable = false)
        @test H(1, [1, 0], [0, 1]) == 4
        @test H(1, [1, 0], [0, 1], v) == 4
        H = Hamiltonian(
            (t, x, p, v) -> t + x[1]^2 + 2p[2] + v[3];
            autonomous = false,
            variable = true
        )
    ...
    @testset "HamiltonianVectorField" begin
        ...
        @test_throws IncorrectArgument HamiltonianVectorField(
            (x, p) -> ([x[1]^2 + 2p[2]], [x[2] - 3p[2]^2]),
            Int64
        )
        @test_throws IncorrectArgument HamiltonianVectorField(
            (x, p) -> ([x[1]^2 + 2p[2]], [x[2] - 3p[2]^2]),
    ...
    @testset "StateConstraint" begin
        ...
        S = StateConstraint(
            (t, x) -> [t + x[1]^2, 2x[2]]; autonomous = false, variable = false)
        @test S(1, [1, -1]) == [2, -2]
        @test S(1, [1, -1], v) == [2, -2]
        S = StateConstraint(
            (t, x, v) -> [t + x[1]^2, 2x[2] + v[3]];
    ...
    @testset "MixedConstraint" begin
        ...
        @test_throws IncorrectArgument MixedConstraint((x, u) -> [2x[2] - u^2, x[1]], Int64)
        M = MixedConstraint(
            (x, u) -> [2x[2] - u^2, x[1]]; autonomous = true, variable = false)
        @test M([1, 0], 1) == [-1, 1]
        t = 1
        v = Real[]
