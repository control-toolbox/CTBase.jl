function test_differential_geometry()
    ...
    @testset "Lifts" begin
        ...
        @testset "HamiltonianLift from Function" begin
            ...
            @test HL(1, [1, 0], [0, 1], 1) == 0
            HL = HamiltonianLift(
                (t, x, v) -> [t + x[1]^2, x[2]^2 + v], NonAutonomous, NonFixed)
            @test HL(1, [1, 0], [0, 1], 1) == 1
        end
    ...
    @testset "Directional derivative of a vector field" begin
        ...
        # nonautonomous, nonfixed, dim 2
        X = VectorField(
            (t, x, v) -> [t + v[1] + v[2] + x[2], -x[1]], NonFixed, NonAutonomous)
    ...
    @testset "poisson bracket of Lifts" begin
        @testset "autonomous case" begin
            ...
            F_ = (x, p) -> p' * f(x)
            G_ = (x, p) -> p' * g(x)
            Test.@test Poisson(F, G)(
                [1, 2, 3], [4, 0, 4])≈Poisson(F_, G_)([1, 2, 3], [4, 0, 4]) atol=1e-6
            Test.@test Poisson(F, G_)(
                [1, 2, 3], [4, 0, 4])≈Poisson(F_, G)([1, 2, 3], [4, 0, 4]) atol=1e-6
        end
        ...
        @testset "nonautonomous case" begin
            ...
            F_ = (t, x, p) -> p' * f(t, x)
            G_ = (t, x, p) -> p' * g(t, x)
            Test.@test Poisson(F, G, NonAutonomous)(
                2, [1, 2, 3],
                [4, 0, 4])≈
            Poisson(F_, G_, NonAutonomous)(2, [1, 2, 3], [4, 0, 4]) atol=1e-6
            Test.@test Poisson(F, G_, NonAutonomous)(
                2, [1, 2, 3],
                [4, 0, 4])≈
        ...
        @testset "autonomous nonfixed case" begin
            ...
            F_ = (x, p, v) -> p' * f(x, v)
            G_ = (x, p, v) -> p' * g(x, v)
            Test.@test Poisson(F, G, NonFixed)(
                [1, 2, 3], [4, 0, 4], 1)≈
            Poisson(F_, G_, NonFixed)([1, 2, 3], [4, 0, 4], 1) atol=1e-6
            Test.@test Poisson(F, G_, NonFixed)(
                [1, 2, 3], [4, 0, 4], 1)≈
        ...
        @testset "nonautonomous nonfixed case" begin
            ...
            Test.@test Poisson(F, G, NonAutonomous, NonFixed)(2, [1, 2, 3], [4, 0, 4],
                1)≈
            Poisson(F_, G_, NonAutonomous, NonFixed)(2, [1, 2, 3], [4, 0, 4], 1) atol=1e-6
            Test.@test Poisson(F, G_, NonAutonomous, NonFixed)(2, [1, 2, 3], [4, 0, 4],
                1)≈
            Poisson(F_, G, NonAutonomous, NonFixed)(2, [1, 2, 3], [4, 0, 4], 1) atol=1e-6
        end
    end
