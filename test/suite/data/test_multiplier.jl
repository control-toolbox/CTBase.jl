...
function test_multiplier()
    Test.@testset "Multiplier Tests" verbose = VERBOSE showtiming = SHOWTIMING begin
        ...
        Test.@testset "Unit: construction and trait extractors" begin
            m1 = Data.Multiplier((x, p) -> x[1]; is_autonomous=true, is_variable=false)
        ...
        Test.@testset "Natural calls" begin
            Test.@test Data.Multiplier((x, p) -> x[1] * p[1])([2.0], [3.0]) == 6.0
            Test.@test Data.Multiplier((t, x, p) -> t + x[1] + p[1]; is_autonomous=false)(
                1.0, [2.0], [3.0]
            ) == 6.0
            Test.@test Data.Multiplier((x, p, v) -> x[1] + p[1] + v[1]; is_variable=true)(
                [2.0], [3.0], [4.0]
            ) == 9.0
            Test.@test Data.Multiplier(
                (t, x, p, v) -> t + x[1] + p[1] + v[1];
                is_autonomous=false,
                is_variable=true,
            )(
        ...
        Test.@testset "Uniform call μ(t, x, p, v)" begin
            ...
            Test.@test Data.Multiplier((x, p) -> x[1] + p[1])(t, x, p, v) == 5.0
            Test.@test Data.Multiplier((t, x, p) -> t + x[1] + p[1]; is_autonomous=false)(
                t, x, p, v
            ) == 6.0
            Test.@test Data.Multiplier((x, p, v) -> x[1] + p[1] + v[1]; is_variable=true)(
        ...
        Test.@testset "Typed constructor" begin
            m = Data.Multiplier((x, p) -> x[1], Traits.Autonomous, Traits.Fixed)
        ...
        Test.@testset "show" begin
            ...
            m2 = Data.Multiplier((t, x, p) -> x[1]; is_autonomous=false)
