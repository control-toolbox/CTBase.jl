function test_model() # 30 55 185
    ...
    @testset "constraint! 2" begin
        ...
        @test_throws UnauthorizedCall constraint!(
            ocp, :final, rg = 1, lb = 0, ub = 0, label = :c)
    end
    ...
    @testset "constraint! 4" begin
        ...
        @test_throws UnauthorizedCall constraint!(
            ocp, :final, rg = 1, lb = 0, ub = 1, label = :c)
    end
    ...
    @testset "constraint! 5" begin
        ...
        control!(ocp, 2)
        constraint!(
            ocp, :boundary; f = (x0, xf) -> x0[1] + xf[1], lb = 0, ub = 0, label = :cb)
    ...
    @testset "constraint! 6" begin
        ...
        control!(ocp, 2)
        constraint!(
            ocp, :boundary; f = (x0, xf) -> x0[1] + xf[1], lb = 0, ub = 1, label = :cb)
    ...
    @testset "constraint! 8" begin
        ...
        control!(ocp, 1)
        constraint!(
            ocp, :boundary; f = (x0, xf, v) -> x0 + xf + v[1], lb = 0, ub = 1, label = :cb)
    ...
    @testset "constraint! 11" begin
        ...
        @test_throws UnauthorizedCall constraint!(
            ocp_error, :control, f = dummy, label = :c1)
        @test_throws UnauthorizedCall constraint!(
            ocp_error, :state, rg = 1:2:3, label = :c2)
        @test_throws IncorrectArgument constraint!(
            ocp_error,
            :state,
            rg = 1:2:3,
    ...
    @testset "nlp_constraints! without variable" begin
        ...
        constraint!(ocp, :state; lb = [0, 1], ub = [1, 2], label = :cs)
        constraint!(
            ocp, :boundary; f = (x0, xf) -> x0[2] + xf[2], lb = 0, ub = 1, label = :cb)
    ...
    @testset "nlp_constraints! with variable" begin
        ...
        constraint!(
            ocp, :state; f = (x, v) -> x + v[1:2], lb = [0, 1], ub = [1, 2], label = :css)
        constraint!(
            ocp, :mixed; f = (x, u, v) -> x[1] + u + v[2], lb = 1, ub = 1, label = :cm)
    ...
    @testset "val vs lb and ub, errors" begin
        ...
        @test_throws UnauthorizedCall constraint!(
            ocp, :control; val = 0, ub = 0, label = :cu)
        @test_throws UnauthorizedCall constraint!(
            ocp,
            :state;
            val = [0, 1],
        ...
        @test_throws UnauthorizedCall constraint!(
            ocp, :control; val = 0, lb = 0, label = :cu)
        @test_throws UnauthorizedCall constraint!(
            ocp,
            :state;
            val = [0, 1],
    ...
    @testset "val vs lb and ub, 2/2" begin
        ...
        constraint!(
            ocp, :boundary; f = (x0, xf, v) -> x0[2] + xf[2] + v[1], val = 2, label = :cb)
