function test_model() # 30 55 185
    ...
    @testset "time, state and control set or not" begin
        for i in 1:7
            ...
            # constraint! 1
            @test_throws UnauthorizedCall constraint!(
                ocp, :initial, rg=1:2:5, lb=[0, 0, 0], ub=[0, 0, 0]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :initial, rg=2:3, lb=[0, 0], ub=[0, 0]
            )
            ...
            # constraint! 3
            @test_throws UnauthorizedCall constraint!(
                ocp, :initial, rg=2:3, lb=[0, 0], ub=[1, 2]
            )
            ...
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, rg=2:3, lb=[0, 0], ub=[1, 2]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :initial, rg=1:2:5, lb=[0, 0, 0], ub=[1, 2, 1]
            )

            # constraint! 4
            @test_throws UnauthorizedCall constraint!(
                ocp, :initial, lb=[0, 0, 0], ub=[1, 2, 1]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :final, lb=[0, 0, 0], ub=[1, 2, 1]
            )
            @test_throws UnauthorizedCall constraint!(ocp, :control, lb=[0, 0], ub=[2, 3])
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, lb=[0, 0, 0], ub=[1, 2, 1]
            )

            # constraint! 5
            # variable independent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :boundary, f=(x0, xf) -> x0[3] + xf[2], lb=0, ub=1
            )

            # variable dependent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :boundary, f=(x0, xf, v) -> x0[3] + xf[2] * v[1], lb=0, ub=1
            )

            # time independent and variable independent ocp
            @test_throws UnauthorizedCall constraint!(ocp, :control, f=u -> 2u, lb=0, ub=1)
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=x -> x - 1, lb=[0, 0, 0], ub=[1, 2, 1]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(x, u) -> x[1] - u, lb=0, ub=1
            )

            # time dependent and variable independent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :control, f=(t, u) -> 2u, lb=0, ub=1
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=(t, x) -> x - t, lb=[0, 0, 0], ub=[1, 2, 1]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(t, x, u) -> x[1] - u, lb=0, ub=1
            )

            # time independent and variable dependent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :control, f=(u, v) -> 2u * v[1], lb=0, ub=1
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=(x, v) -> x - v[1], lb=[0, 0, 0], ub=[1, 2, 1]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(x, u, v) -> x[1] - v[2] * u, lb=0, ub=1
            )

            # time dependent and variable dependent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :control, f=(t, u, v) -> 2u + v[2], lb=0, ub=1
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=(t, x, v) -> x - t * v[1], lb=[0, 0, 0], ub=[1, 2, 1]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(t, x, u, v) -> x[1] * v[2] - u, lb=0, ub=1
            )

            # constraint! 6
            # variable independent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :boundary, f=(x0, xf) -> x0[3] + xf[2], lb=0, ub=0
            )

            # variable dependent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :boundary, f=(x0, xf, v) -> x0[3] + xf[2] * v[1], lb=0, ub=0
            )

            # time independent and variable independent ocp
            @test_throws UnauthorizedCall constraint!(ocp, :control, f=u -> 2u, lb=1, ub=1)
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=x -> x - 1, lb=[0, 0, 0], ub=[0, 0, 0]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(x, u) -> x[1] - u, lb=0, ub=0
            )

            # time dependent and variable independent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :control, f=(t, u) -> 2u, lb=1, ub=1
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=(t, x) -> x - t, lb=[0, 0, 0], ub=[0, 0, 0]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(t, x, u) -> x[1] - u, lb=0, ub=0
            )

            # time independent and variable dependent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :control, f=(u, v) -> 2u * v[1], lb=1, ub=1
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=(x, v) -> x - v[2], lb=[0, 0, 0], ub=[0, 0, 0]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(x, u) -> x[1] - u + v[1], lb=0, ub=0
            )

            # time dependent and variable dependent ocp
            @test_throws UnauthorizedCall constraint!(
                ocp, :control, f=(t, u, v) -> 2u - t * v[2], lb=1, ub=1
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :state, f=(t, x, v) -> x - t + v[1], lb=[0, 0, 0], ub=[0, 0, 0]
            )
            @test_throws UnauthorizedCall constraint!(
                ocp, :mixed, f=(t, x, u, v) -> x[1] - u * v[1], lb=0, ub=0
            )
        end
    end
    ...
    @testset "constraint! 1" begin
        ...
        @test_throws IncorrectArgument constraint!(
            ocp, :initial, lb=[0, 1], ub=[0, 1], label=:c0
        )
    ...
    @testset "constraint! 2" begin
        ...
        xf = [1, 2]
        @test_throws IncorrectArgument constraint!(
            ocp, :initial, rg=2, lb=x0, ub=x0, label=:c0
        )
        @test_throws IncorrectArgument constraint!(
            ocp, :final, rg=2, lb=xf, ub=xf, label=:cf
        )
    ...
    @testset "constraint! 4" begin
        ...
        control!(ocp, 2)
        @test_throws IncorrectArgument constraint!(
            ocp, :initial, rg=2, lb=[0, 1], ub=[1, 2], label=:c0
        )
        @test_throws IncorrectArgument constraint!(
            ocp, :final, rg=2, lb=[1, 2], ub=[2, 3], label=:cf
        )
        @test_throws IncorrectArgument constraint!(
            ocp, :control, rg=2, lb=[0, 1], ub=[1, 2], label=:cu
        )
        @test_throws IncorrectArgument constraint!(
            ocp, :state, rg=2, lb=[0, 1], ub=[1, 2], label=:cs
        )
    ...
    @testset "constraint! 5" begin
        ...
        @test_throws UnauthorizedCall constraint!(
            ocp, :control, f=u -> u, lb=0, ub=1, label=:c
        )
    end
    ...
    @testset "constraint! 7" begin
        ...
        constraint!(
            ocp, :variable; f=v -> v .^ 2, lb=[0, 0, 0, 0], ub=[1, 0, 1, 0], label=:eq5
        )
    ...
    @testset "constraint! 8" begin
        ...
        constraint!(
            ocp, :variable; f=v -> v .^ 2, lb=[0, 0, 0, 0], ub=[0, 0, 0, 0], label=:eq5
        )
    ...
    @testset "constraint! 11" begin
        ...
        @test_throws IncorrectArgument constraint!(
            ocp_error, :state, rg=1:2:3, f=dummy, lb=[0, 0], ub=[0, 0], label=:c3
        )
        @test_throws IncorrectArgument constraint!(
            ocp_error, :state, f=dummy, rg=1:2:3, lb=[0, 0], ub=[0, 0], label=:c4
        )
        @test_throws IncorrectArgument constraint!(
            ocp_error, :foo, lb=[0, 0], ub=[0, 0], label=:c5
        )
    end
    ...
    @testset "nlp_constraints! without variable" begin
        ...
        (ξl, ξ, ξu), (ηl, η, ηu), (ψl, ψ, ψu), (ϕl, ϕ, ϕu), (θl, θ, θu), (ul, uind, uu), (xl, xind, xu), (vl, vind, vu) = nlp_constraints!(
            ocp
        )
    ...
    @testset "nlp_constraints! with variable" begin
        ...
        constraint!(ocp, :state; lb=[0, 1], ub=[1, 2], label=:cs)
        constraint!(
            ocp, :boundary; f=(x0, xf, v) -> x0[2] + xf[2] + v[1], lb=0, ub=1, label=:cb
        )
    ...
    @testset "nlp_constraints! without variable (in place)" begin
        ...
        constraint!(
            ocp, :state; f=(r, x) -> (r[:] .= x; nothing), lb=[0, 1], ub=[1, 2], label=:css
        )
        constraint!(
            ocp, :mixed; f=(r, x, u) -> (r[:] .= x[1] + u; nothing), lb=1, ub=1, label=:cm
        )
    ...
    @testset "nlp_constraints! with variable (in place)" begin
        ...
        constraint!(ocp, :variable; rg=3, lb=2, ub=3, label=:cv3)
        constraint!(
            ocp, :variable; f=(r, v) -> (r[:] .= v[3]^2; nothing), lb=0, ub=1, label=:cv4
        )
    ...
    @testset "val vs lb and ub, errors" begin
        ...
        # error val with ub
        @test_throws UnauthorizedCall constraint!(
            ocp, :initial; rg=Index(2), val=10, ub=10, label=:ci
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :final; rg=Index(1), val=1, ub=1, label=:cf
        )
        @test_throws UnauthorizedCall constraint!(ocp, :control; val=0, ub=0, label=:cu)
        @test_throws UnauthorizedCall constraint!(
            ocp, :state; val=[0, 1], ub=[0, 1], label=:cs
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :boundary; f=(x0, xf, v) -> x0[2] + xf[2] + v[1], val=2, ub=2, label=:cb
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :control; f=(u, v) -> u + v[2], val=20, ub=20, label=:cuu
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :state; f=(x, v) -> x + v[1:2], val=[100, 101], ub=[100, 101], label=:css
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :mixed; f=(x, u, v) -> x[1] + u + v[2], val=-1, ub=-1, label=:cm
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; val=[5, 5, 5, 5], ub=[5, 5, 5, 5], label=:cv1
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; rg=1:2, val=[10, 20], ub=[10, 20], label=:cv2
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; rg=Index(3), val=1000, ub=1000, label=:cv3
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; f=v -> v[3]^2, val=-10, ub=-10, label=:cv4
        )
        ...
        # error val with lb
        @test_throws UnauthorizedCall constraint!(
            ocp, :initial; rg=Index(2), val=10, lb=10, label=:ci
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :final; rg=Index(1), val=1, lb=1, label=:cf
        )
        @test_throws UnauthorizedCall constraint!(ocp, :control; val=0, lb=0, label=:cu)
        @test_throws UnauthorizedCall constraint!(
            ocp, :state; val=[0, 1], lb=[0, 1], label=:cs
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :boundary; f=(x0, xf, v) -> x0[2] + xf[2] + v[1], val=2, lb=2, label=:cb
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :control; f=(u, v) -> u + v[2], val=20, lb=20, label=:cuu
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :state; f=(x, v) -> x + v[1:2], val=[100, 101], lb=[100, 101], label=:css
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :mixed; f=(x, u, v) -> x[1] + u + v[2], val=-1, lb=-1, label=:cm
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; val=[5, 5, 5, 5], lb=[5, 5, 5, 5], label=:cv1
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; rg=1:2, val=[10, 20], lb=[10, 20], label=:cv2
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; rg=Index(3), val=1000, lb=1000, label=:cv3
        )
        @test_throws UnauthorizedCall constraint!(
            ocp, :variable; f=v -> v[3]^2, val=-10, lb=-10, label=:cv4
        )
    end
    ...
    @testset "val vs lb and ub, 1/2" begin
        ...
        constraint!(
            ocp, :boundary; f=(x0, xf, v) -> x0[2] + xf[2] + v[1], lb=2, ub=2, label=:cb
        )
        constraint!(ocp, :control; f=(u, v) -> u + v[2], lb=20, ub=20, label=:cuu)
        constraint!(
            ocp, :state; f=(x, v) -> x + v[1:2], lb=[100, 101], ub=[100, 101], label=:css
        )
        constraint!(ocp, :mixed; f=(x, u, v) -> x[1] + u + v[2], lb=-1, ub=-1, label=:cm)
    ...
    @testset "val vs lb and ub, 2/2 (in place)" begin
        ...
        constraint!(
            ocp, :control; f=(r, u, v) -> (r[:] .= u + v[2]; nothing), val=20, label=:cuu
        )
        constraint!(
            ocp,
            :state;
        ...
        constraint!(ocp, :variable; rg=Index(3), val=1000, label=:cv3)
        constraint!(
            ocp, :variable; f=(r, v) -> (r[:] .= v[3]^2; nothing), val=-10, label=:cv4
        )
    ...
    @testset "objective!" begin
        ...
        @test_throws UnauthorizedCall objective!(
            ocp, :bolza, (t0, x0, tf, xf) -> 0.5x0^2, (x, u) -> 0.5u^2
        )
