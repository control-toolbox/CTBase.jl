...
@testset "variable!" begin
    ...
    @test_throws UnauthorizedCall constraint!(ocp, :variable, 2:3, [ 0, 3 ], [ 0, 3 ])
    ...
    @test_throws UnauthorizedCall constraint!(ocp, :variable, [ 3, 0, 1 ], [ 3, 0, 1 ])
...
@testset "time, state and control set or not" begin
    ...
    # constraint! 1
    @test_throws UnauthorizedCall constraint!(ocp, :initial, 1:2:5, [ 0, 0, 0 ], [ 0, 0, 0 ])
    @test_throws UnauthorizedCall constraint!(ocp, :initial, 2:3, [ 0, 0 ], [ 0, 0 ])
    @test_throws UnauthorizedCall constraint!(ocp, :final, Index(2), 0, 0)

    # constraint! 2
    @test_throws UnauthorizedCall constraint!(ocp, :initial, [ 0, 0 ], [ 0, 0 ])
    @test_throws UnauthorizedCall constraint!(ocp, :final, 2, 2) # if the state is of dimension 1
    ...
    # constraint! 6
    # variable independent ocp
    @test_throws UnauthorizedCall  constraint!(ocp, :boundary, (x0, xf) -> x0[3]+xf[2], 0, 0)

    # variable dependent ocp
    @test_throws UnauthorizedCall  constraint!(ocp, :boundary, (x0, xf, v) -> x0[3]+xf[2]*v[1], 0, 0)

    # time independent and variable independent ocp
    @test_throws UnauthorizedCall  constraint!(ocp, :control, u -> 2u, 1, 1)
    @test_throws UnauthorizedCall  constraint!(ocp, :state, x -> x-1, [ 0, 0, 0 ], [ 0, 0, 0 ])
    @test_throws UnauthorizedCall  constraint!(ocp, :mixed, (x, u) -> x[1]-u, 0, 0)

    # time dependent and variable independent ocp
    @test_throws UnauthorizedCall  constraint!(ocp, :control, (t, u) -> 2u, 1, 1)
    @test_throws UnauthorizedCall  constraint!(ocp, :state, (t, x) -> x-t, [ 0, 0, 0 ], [ 0, 0, 0 ])
    @test_throws UnauthorizedCall  constraint!(ocp, :mixed, (t, x, u) -> x[1]-u, 0, 0)

    # time independent and variable dependent ocp
    @test_throws UnauthorizedCall  constraint!(ocp, :control, (u, v) -> 2u*v[1], 1, 1)
    @test_throws UnauthorizedCall  constraint!(ocp, :state, (x, v) -> x-v[2], [ 0, 0, 0 ], [ 0, 0, 0 ])
    @test_throws UnauthorizedCall  constraint!(ocp, :mixed, (x, u) -> x[1]-u+v[1], 0, 0)

    # time dependent and variable dependent ocp
    @test_throws UnauthorizedCall  constraint!(ocp, :control, (t, u, v) -> 2u-t*v[2], 1, 1)
    @test_throws UnauthorizedCall  constraint!(ocp, :state, (t, x, v) -> x-t+v[1], [ 0, 0, 0 ], [ 0, 0, 0 ])
    @test_throws UnauthorizedCall  constraint!(ocp, :mixed, (t, x, u, v) -> x[1]-u*v[1], 0, 0)
...
@testset "constraint! 1" begin
    ...
    @test_throws IncorrectArgument constraint!(ocp, :initial, [0, 1], [0, 1], :c0)
    constraint!(ocp, :initial, 0, 0, :c0)
    constraint!(ocp, :final, 1, 1, :cf)
    @test constraint(ocp, :c0)(12, ∅) == 12
    @test constraint(ocp, :cf)(∅, 12) == 12

    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 2); control!(ocp, 1)
    constraint!(ocp, :initial, [0, 1], [0, 1], :c0)
    constraint!(ocp, :final, [1, 2], [1, 2], :cf)
    @test constraint(ocp, :c0)([12, 13], ∅) == [12, 13]
    @test constraint(ocp, :cf)(∅, [12, 13]) == [12, 13]

    # constraint already exists
    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 1); control!(ocp, 1)
    constraint!(ocp, :initial, 0, 0, :c)
    @test_throws UnauthorizedCall constraint!(ocp, :final, 0, 0, :c)
...
@testset "constraint! 2" begin
    ...
    x0 = 0
    xf = 1
    constraint!(ocp, :initial, Index(1), x0, x0, :c0)
    constraint!(ocp, :final, Index(1), xf, xf, :cf)
    ...
    x0 = [0, 1]
    xf = [1, 2]
    @test_throws IncorrectArgument constraint!(ocp, :initial, Index(2), x0, x0, :c0)
    @test_throws IncorrectArgument constraint!(ocp, :final, Index(2), xf, xf, :cf)

    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 2); control!(ocp, 1)
    x  = [12, 13]
    x0 = [0, 1]
    xf = [1, 2]
    constraint!(ocp, :initial, 1:2, x0, x0, :c0)
    constraint!(ocp, :final, 1:2, xf, xf, :cf)
    @test constraint(ocp, :c0)(x, ∅) == x[1:2]
    @test constraint(ocp, :cf)(∅, x) == x[1:2]

    # constraint already exists
    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 2); control!(ocp, 1)
    constraint!(ocp, :initial, Index(1), 0, 0, :c)
    @test_throws UnauthorizedCall constraint!(ocp, :final, Index(1), 0, 0, :c)
...
@testset "constraint! 5" begin
    ...
    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 1); control!(ocp, 1)
    constraint!(ocp, :boundary, (x0, xf) -> x0+xf, 0, 0, :cb)
    constraint!(ocp, :control, u->u, 0, 0, :cu)
    constraint!(ocp, :state, x->x, 0, 0, :cs)
    ...
    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 2); control!(ocp, 2)
    constraint!(ocp, :boundary, (x0, xf) -> x0[1]+xf[1], 0, 0, :cb)
    constraint!(ocp, :control, u->u[1], 0, 0, :cu)
    constraint!(ocp, :state, x->x[1], 0, 0, :cs)
    ...
    constraint!(ocp, :boundary, (x0, xf) -> [x0[1]+xf[1], x0[2]+xf[2]], [0, 0], [0, 0], :cb)
    constraint!(ocp, :control, u->u[1:2], [0, 0], [0, 0], :cu)
    constraint!(ocp, :state, x->x[1:2], [0, 0], [0, 0], :cs)
    constraint!(ocp, :mixed, (x,u)->[x[1]+u[1], x[2]+u[2]], [0, 0], [0, 0], :cm)
...
@testset "constraint! 8" begin
    ...
    constraint!(ocp, :variable, [ 0, 0, 0, 0 ], [ 0, 0, 0, 0 ], :eq1)
    constraint!(ocp, :variable, Index(1), 0, 0, :eq2)
    constraint!(ocp, :variable, 1:2, [ 0, 0 ], [ 0, 0 ], :eq3)
    constraint!(ocp, :variable, 1:2:4, [ 0, 0 ], [ 0, 0 ], :eq4)
    constraint!(ocp, :variable, v -> v.^2, [ 0, 0, 0, 0 ], [ 0, 0, 0, 0], :eq5)
...
@testset "constraint! 11" begin
    ...
    constraint!(ocp, :control, dummy, 1, 1, :c1)
    ...
    constraint!(ocp_ter, :variable, 1, 1, :c0)
...
@testset "nlp_constraints without variable" begin
    ...
    ocp = Model(); time!(ocp, 0, 1); state!(ocp, 2); control!(ocp, 1)
    constraint!(ocp, :initial, Index(2), 10, 10, :ci)
    constraint!(ocp, :final, Index(1), 1, 1, :cf)
    constraint!(ocp, :control, 0, 1, :cu)
    constraint!(ocp, :state, [0, 1], [1, 2], :cs)
