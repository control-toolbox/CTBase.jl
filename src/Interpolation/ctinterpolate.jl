...
This creates a one-dimensional linear [`CTBase.Interpolation.Interpolant`](@extref) with flat extrapolation beyond
...
# Returns
A callable [`CTBase.Interpolation.Interpolant`](@extref) that can be evaluated at new points.
...
This creates a right-continuous piecewise-constant [`CTBase.Interpolation.Interpolant`](@extref): the value at knot
