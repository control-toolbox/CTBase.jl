...
# output: vectorial
@eval function (F::$(function_name){dimension_usage})(t0::Time, 
...
# handling time dependence and scalar / vectorial usage
# output: scalar
@eval function (F::$(function_name){time_dependence, dimension_usage})(t::Time, 
...
@eval @ctfunction_td_sv $(function_name), $(abstract_heritance)
# handling time dependence and scalar / vectorial usage
@eval function (F::$(function_name){time_dependence, dimension_usage})(t::Time, x::State, p::Adjoint, args...; kwargs...) where {time_dependence, dimension_usage}
    args_ = time_dependence==:autonomous ? () : (t, )
    if dimension_usage==:scalar && length(x)==1
        args_ = (args_..., x[1], p[1])
    else
...
# handling time dependence and scalar / vectorial usage
# output: vectorial
@eval function (F::$(function_name){time_dependence, dimension_usage})(t::Time, x::State, args...; kwargs...) where {time_dependence, dimension_usage}
    t_ = time_dependence==:autonomous ? () : (t, )
    x_ = (dimension_usage==:scalar && length(x)==1) ? (x[1], ) : (x, )
...
# handling time dependence and scalar / vectorial usage
# output: vectorial
@eval function (F::$(function_name){time_dependence, dimension_usage})(t::Time, u::Control, args...; kwargs...) where {time_dependence, dimension_usage}
    t_ = time_dependence==:autonomous ? () : (t, )
    u_ = (dimension_usage==:scalar && length(u)==1) ? (u[1], ) : (u, )
...
# handling time dependence and scalar / vectorial usage
# output: vectorial
@eval function (F::$(function_name){time_dependence, dimension_usage})(t::Time, x::State, u::Control, args...; kwargs...) where {time_dependence, dimension_usage}
    t_ = time_dependence==:autonomous ? () : (t, )
    x_ = (dimension_usage==:scalar && length(x)==1) ? (x[1], ) : (x, )
    u_ = (dimension_usage==:scalar && length(u)==1) ? (u[1], ) : (u, )
    y = F.f(t_..., x_..., u_..., args...; kwargs...)
