...
under `promote_type`. Combined with the `T` floor in [`CTBase.Core.to_out_of_place`](@extref),
non-numeric-only calls keep `T`.

See also: [`CTBase.Core.to_out_of_place`](@extref).
"""
...
[`CTBase.Core._promote_arg_eltype`](@extref)) so that `ForwardDiff.Dual` values are accommodated
