...
function __check_dependences(dependences::Tuple{Vararg{DataType}})
    ...
    size(filter(p->!(p<:Union{TimeDependence,VariableDependence}),dependences),1) > 0 && throw(IncorrectArgument("the wrong of arguments, possible arguments are : NonAutonomous, Autonomous, NonVariable, Variable"))
end
