...
[`CTBase.Plotting.lower`](@extref) step turns a `Panel` into [`CTBase.Plotting.Leaf`](@extref)/[`CTBase.Plotting.Axes`](@extref) nodes.
...
Return the number of components (columns) of a [`CTBase.Plotting.Panel`](@extref).
"""
...
Return the style to apply to component `i` of a [`CTBase.Plotting.Panel`](@extref): its own per-component
...
Default overall layout for [`CTBase.Plotting.lower`](@extref): `:split` (one subplot per component).
Alternative: `:group`.
"""
__layout() = :split
...
Default time-axis handling for [`CTBase.Plotting.lower`](@extref): `:default` (real time).
...
Compact one-line display of a [`CTBase.Plotting.Panel`](@extref).
...
Pretty display of a [`CTBase.Plotting.Panel`](@extref), showing its title, labels,
and data dimensions.
