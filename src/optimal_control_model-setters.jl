...
function variable!(
    ocp::OptimalControlModel, q::Dimension, name::Symbol, components_names::Vector{Symbol}
)
    return variable!(ocp, q, string(name), string.(components_names))
end

function variable!(
    ocp::OptimalControlModel, q::Dimension, name::Symbol, components_names::Vector{String}
)
...
function state!(
    ocp::OptimalControlModel, n::Dimension, name::Symbol, components_names::Vector{Symbol}
)
    return state!(ocp, n, string(name), string.(components_names))
end

function state!(
    ocp::OptimalControlModel, n::Dimension, name::Symbol, components_names::Vector{String}
)
...
function control!(
    ocp::OptimalControlModel, m::Dimension, name::Symbol, components_names::Vector{Symbol}
)
    return control!(ocp, m, string(name), string.(components_names))
end

function control!(
    ocp::OptimalControlModel, m::Dimension, name::Symbol, components_names::Vector{String}
)
    ...
    @match (t0, ind0, tf, indf) begin
        (::Time, ::Nothing, ::Time, ::Nothing) => begin # (t0, tf)
            ...
            ocp.initial_time_name =
                t0 isa Int ? string(t0) : string(round(t0; digits=2))
        ...
        (::RangeConstraint, ::Nothing, ::ctVector, ::ctVector) => begin
            ...
            fun_rg = @match type begin
                :initial => if is_in_place(ocp)
                    ...
                    if V == Fixed
                        BoundaryConstraint((x0, xf) -> x0[rg], V)
                    else
                ...
                :final => if is_in_place(ocp)
                    ...
                    if V == Fixed
                        BoundaryConstraint((x0, xf) -> xf[rg], V)
                    else
...
"""
function dynamics!(
    ocp::OptimalControlModel{T,V}, f::Function
) where {T<:TimeDependence,V<:VariableDependence}
