...
"""
__variable_components_names(q::Dimension, name::String) = q > 1 ?
                                                          [name * ctindices(i)
                                                           for i in range(1, q)] :
                                                          [name]
...
"""
__state_components_names(n::Dimension, name::String) = n > 1 ?
                                                       [name * ctindices(i)
                                                        for i in range(1, n)] :
                                                       [name]
...
"""
__control_components_names(m::Dimension, name::String) = m > 1 ?
                                                         [name * ctindices(i)
                                                          for i in range(1, m)] :
                                                         [name]
...
function __init_interpolation()
    return (T, U) -> Interpolations.linear_interpolation(
        T, U; extrapolation_bc = Interpolations.Line())
end
